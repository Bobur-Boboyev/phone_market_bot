...
from bot.models.smartphones import smartphones_list
...
class MessageHandler:
    ...
    def show_phone(self, update: Update, context: CallbackContext, index=0):
        ...
        update.message.reply_photo(
            photo=phone.photo, caption=caption, reply_markup=reply_markup
        )
    ...
    def phone_callback(self, update: Update, context: CallbackContext):
        ...
        if action == "prev" and index > 0:
            new_index = index - 1
        elif action == "next" and index < len(phones) - 1:
            new_index = index + 1
        elif action == "buy":
            query.edit_message_caption(
                f"Siz {phones[index].name} ni xarid qildingiz 🛒"
            )
            return
        ...
        keyboard = [
            [
                ...
                InlineKeyboardButton(
                    "Sotib olish 🛒", callback_data=f"buy_{new_index}"
                ),
